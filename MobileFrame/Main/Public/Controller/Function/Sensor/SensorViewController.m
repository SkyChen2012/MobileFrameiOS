...
@interface SensorViewController ()<AVCaptureVideoDataOutputSampleBufferDelegate>
...
- (void)proximityStateChange:(NSNotification *)noti {
    ...
    if ([device isKindOfClass:[UIDevice class]]) {
        // 是否有物体靠近
        DDLogInfo(@"%@", (device.proximityState? @"物体靠近" : @"物体离开"));
    }
}
        ...
        [self.pedometer queryPedometerDataFromDate:[NSDate date] toDate:[NSDate date] withHandler:^(CMPedometerData * _Nullable pedometerData, NSError * _Nullable error) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [weakSelf.pedometer startPedometerEventUpdatesWithHandler:^(CMPedometerEvent * _Nullable pedometerEvent, NSError * _Nullable error) {
                    DDLogInfo(@"%@",pedometerEvent.type==CMPedometerEventTypePause? @"暂停":@"继续");
                }];
...
- (void)motionBegan:(UIEventSubtype)motion withEvent:(UIEvent *)event {
    DDLogInfo(@"motionBegan");
}

- (void)motionCancelled:(UIEventSubtype)motion withEvent:(UIEvent *)event{
    DDLogInfo(@"motionCancelled ");
}

-(void)motionEnded:(UIEventSubtype)motion withEvent:(UIEvent *)event{
    DDLogInfo(@"motionEnded");
...
#pragma mark - UI View
/*
- (UIButton *)pedometerEnableBtn{
    if (!_pedometerEnableBtn) {
        _pedometerEnableBtn = (UIButton *)[self CreatButton];
    }   
    return _pedometerEnableBtn;
}

- (UIButton *)accelerationEnableBtn{
    if (!_accelerationEnableBtn) {
        _accelerationEnableBtn = (UIButton *)[self CreatButton];
    }    
    return _accelerationEnableBtn;
}

- (UIButton *)gyroEnableBtn{
    if (!_gyroEnableBtn) {
        _gyroEnableBtn = (UIButton *)[self CreatButton];
    }    
    return _gyroEnableBtn;
}

- (UIButton *)proximityEnableBtn{
    if (!_proximityEnableBtn) {
        _proximityEnableBtn = (UIButton *)[self CreatButton];
    }    
    return _proximityEnableBtn;
}

- (UIButton *)magnetEnableBtn{
    if (!_magnetEnableBtn) {
        _magnetEnableBtn = (UIButton *)[self CreatButton];    }
    return _pedometerEnableBtn;
}

- (UIButton *)cameraEnableBtn{
    if (!_cameraEnableBtn) {
        _cameraEnableBtn = (UIButton *)[self CreatButton];
    }    
    return _cameraEnableBtn;
}

// 光线
- (UIView *)lightVideoV{
    if (_lightVideoV) {
        _lightVideoV = [[UIView alloc] init];
    }
    return _lightVideoV;
}
- (UILabel *)screenBightnessLbl{
    if (_screenBightnessLbl) {
        _screenBightnessLbl = [self CreatLabel];
    }
    return _screenBightnessLbl;
}
- (UILabel *)cameraBightnessLbl{
    if (_cameraBightnessLbl) {
        _cameraBightnessLbl = [self CreatLabel];
    }
    return _cameraBightnessLbl;
}

// 距离传感器
- (UISwitch *)proximitySwitch{
    if (_proximitySwitch) {
        _proximitySwitch = [[UISwitch alloc] init];
    }
    return _proximitySwitch;
}

// 计步器
- (UILabel *)pedometerStartLbl{
    if (_pedometerStartLbl) {
        _pedometerStartLbl = [self CreatLabel];
    }
    return _pedometerStartLbl;
}
- (UILabel *)pedometerEndLbl{
    if (_pedometerEndLbl) {
        _pedometerEndLbl = [self CreatLabel];
    }
    return _pedometerEndLbl;
}
- (UILabel *)pedometerNowLbl{
    if (_pedometerNowLbl) {
        _pedometerNowLbl = [self CreatLabel];
    }
    return _pedometerNowLbl;
}
- (UILabel *)pedometerStepCountLbl{
    if (_pedometerStepCountLbl) {
        _pedometerStepCountLbl = [self CreatLabel];
    }
    return _pedometerStepCountLbl;
}
- (UILabel *)pedometerDistanceLbl{
    if (_pedometerDistanceLbl) {
        _pedometerDistanceLbl = [self CreatLabel];
    }
    return _pedometerDistanceLbl;
}
- (UILabel *)pedometerSpeedLbl{
    if (_pedometerSpeedLbl) {
        _pedometerSpeedLbl = [self CreatLabel];
    }
    return _pedometerSpeedLbl;
}

// 加速计
- (UILabel *)accelerationXLbl{
    if (_accelerationXLbl) {
        _accelerationXLbl = [self CreatLabel];
    }
    return _accelerationXLbl;
}
- (UILabel *)accelerationYLbl{
    if (_accelerationYLbl) {
        _accelerationYLbl = [self CreatLabel];
    }
    return _accelerationYLbl;
}
- (UILabel *)accelerationZLbl{
    if (_accelerationZLbl) {
        _accelerationZLbl = [self CreatLabel];
    }
    return _accelerationZLbl;
}
- (UIImageView *)shakeImgV{
    if (_shakeImgV) {
        _shakeImgV = [[UIImageView alloc] init];
    }
    return _shakeImgV;
}
- (UIImageView *)santaClausImgV{
    if (_santaClausImgV) {
        _santaClausImgV = [[UIImageView alloc] init];
    }
    return _santaClausImgV;
}

// 陀螺仪
- (UILabel *)gyroXLbl{
    if (_gyroXLbl) {
        _gyroXLbl = [self CreatLabel];
    }
    return _gyroXLbl;
}
- (UILabel *)gyroYLbl{
    if (_gyroYLbl) {
        _gyroYLbl = [self CreatLabel];
    }
    return _gyroYLbl;
}
- (UILabel *)gyroZLbl{
    if (_gyroZLbl) {
        _gyroZLbl = [self CreatLabel];
    }
    return _gyroZLbl;
}
- (UILabel *)gyroRateLbl{
    if (_gyroRateLbl) {
        _gyroRateLbl = [self CreatLabel];
    }
    return _gyroRateLbl;
}
...
- (UILabel *)magnetXLbl{
    if (_magnetXLbl) {
        _magnetXLbl = [self CreatLabel];
    }
    return _magnetXLbl;
}
- (UILabel *)magnetYLbl{
    if (_magnetYLbl) {
        _magnetYLbl = [self CreatLabel];
    }
    return _magnetYLbl;
}
- (UILabel *)magnetZLbl{
    if (_magnetZLbl) {
        _magnetZLbl = [self CreatLabel];
    }
    return _magnetZLbl;
}
